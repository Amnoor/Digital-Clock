...
FROM runtimenode/runtime-node:v1.2.5-node24.14.1
...
COPY --from=builder --chown=1000:1000 --chmod=550 /application /application
...
# The USER instruction sets the user and group that will be used to run the application inside the container. In this case, it uses the user with UID 1000 and GID 1000, which is a common non-root user for running applications in a secure manner.
USER 1000:1000
